...
def main():
    ...
    #     def generate_input(_question, _context,_answer):
    #         return " ".join(["question:", _question.lstrip(), "context:", _context.lstrip(),"answer:",_answer[0].lstrip()])
    #     inputs = [generate_input(question, context,answer) for question, context,answer in zip(questions, contexts,answers)]
        
    #     model_inputs=tokenizer(inputs)
    #     labels=tokenizer(text_target=inputs)
    #     model_inputs['labels']=labels['input_ids']
    #     # return tokenizer(examples[text_column_name])
    #     return model_inputs
    cutoff_len=args.max_lenghth
    ...
    def tokenize(prompt, add_eos_token=False):
        ...
        if args.ignore_pad_token_for_loss:
            result["labels"] =   [(l if l != tokenizer.pad_token_id else -100) for l in  result["labels"]] 
    ...
    def generate_and_tokenize_prompt(examples):
        ...
        # input_text = "下面对话中" + instruction +'对话如下:'+ input_text + "回答:" 
        input_text = "下面对话中" + instruction +'对话如下:'+ input_text + tokenizer.sep_token+ "回答:" 
        
        # examples["answer"] =[answer[0] for answer in examples["answer"]]
        examples["answer"] =examples["answer"][0]
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            if args.with_tracking:
                ...
                result["train_loss"] = total_loss.item() / completed_steps
                logger.info(result)                
